...
#   #tmux send -t $tmux_name "nohup ./earthd start --home=./workspace/earth/validator${i} > output 2>&1 & " Enter
  ...
  startID=`expr \( $i % 4 \) \* 4  `
  tmux send -t $tmux_name "nohup ./tpsclient-new $startID CrossChain $idCount 0 > output$idCount 2>&1 &" Enter
  startID=`expr \( $startID + 1 \)  `
  tmux send -t $tmux_name "nohup ./tpsclient-new $startID CrossChain $idCount 0 > output1 2>&1 & " Enter
  startID=`expr \( $startID + 1 \)  `
  tmux send -t $tmux_name "nohup ./tpsclient-new $startID CrossChain $idCount 0 > output2 2>&1 & " Enter
  startID=`expr \( $startID + 1 \)  `
  tmux send -t $tmux_name "nohup ./tpsclient-new $startID CrossChain $idCount 0 > output3 2>&1 & " Enter
elif [ "$1" == "update" ]; then
