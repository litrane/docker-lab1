...
host_string=(" root@172.25.149.108")
name="deploy-theta2"
...
do
  ...
  tmux send -t $tmux_name "cd ~/docker-lab4/theta" Enter
  tmux send -t $tmux_name "nohup ./theta-eth-rpc-adaptor start --config=./thetasub_eth_rpc_adaptor  > adaoutput 2>&1 &  " Enter
  tmux send -t $tmux_name "nohup ./lab4 start --config=./subchain_ipdps_relayer/DSN_$idCount/node${val}/ --password=qwe  > ./output.log 2>&1 & " Enter
  echo "DSN_$idCount"
elif [ "$1" == "run" ]; then  
  tmux send -t $tmux_name "nohup ./relayer-aliclient ${idCount} 10 1 > subwit.log 2>&1 &" Enter
  if [ "$idCount" == "360001" ]; then
    tmux send -t $tmux_name "nohup ./relayer-aliclient 0 > mainwit.log 2>&1 &" Enter
  fi
elif [ "$1" == "update" ]; then
  tmux send -t $tmux_name " ps -ef | grep theta | grep -v grep | awk '{print \$2}' | xargs kill -9" Enter
  tmux send -t $tmux_name "cd ~/docker-lab1" Enter
  ...
  tmux send -t $tmux_name "rm -rf docker-lab4" Enter
  tmux send -t $tmux_name "rm -rf docker-lab4-bak" Enter
elif [ "$1" == "stop" ]; then
  ...
  tmux send -t $tmux_name "ps -ef | grep lab4 | grep -v grep | awk '{print \$2}' | xargs kill -9" Enter
elif [ "$1" == "kw" ]; then
