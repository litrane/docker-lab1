...
do
  ...
  #tmux send -t $tmux_name "nohup ./lab4 start --config=./subchain_ipdps/DSN_$idCount/node${val}/ --password=qwe  > ./output.log 2>&1 & " Enter
  echo "DSN_$idCount"
elif [ "$1" == "run" ]; then  
  tmux send -t $tmux_name "nohup ./aliclient ${idCount} 7 1 > subwit.log 2>&1 &" Enter
